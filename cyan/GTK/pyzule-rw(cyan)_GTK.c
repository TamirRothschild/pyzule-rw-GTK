...
// the new and improved pyzule (aka Cyan) NOW with a full(mostly) GUi app based on GTK with full GTK support; made with love. 
...
void run_pyzule(GtkWidget *widget, gpointer data) {
    ...
    if (file_path && output_path) {
        char command[2048];
        snprintf(command, sizeof(command), "cyan -i %s -o %s -f %s %s", file_path, output_path, files_arg, args); // was just changing it from pyzule > cyan; lol.
        system(command);
        g_free(file_path);
        g_free(output_path);
    } else {
...
int main(int argc, char *argv[]) {
    ...
    gtk_window_set_title(GTK_WINDOW(window), "Cyan-GUi");
    ...
    file_button = gtk_button_new_with_label("Select .ipa file (Source ipa)");
    ...
    files_button = gtk_button_new_with_label("Select files to inject (deb, dylib...etc)");
